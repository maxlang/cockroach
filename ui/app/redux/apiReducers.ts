...
function rollupStoreMetrics(res: state.NodesResponseMessage): NodeStatus[] {
...
const nodesReducerObj = new CachedDataReducer((req: state.NodesRequestMessage, timeout?: moment.Duration) => api.getNodes(req, timeout).then(rollupStoreMetrics), "nodes", moment.duration(10, "s"));
...
export const databaseRequestToID = (req: state.DatabaseDetailsRequestMessage): string => req.database;
...
export const tableRequestToID = (req: state.TableDetailsRequestMessage | state.TableStatsRequestMessage): string => generateTableID(req.database, req.table);
...
export default combineReducers<state.APIReducersState>({
