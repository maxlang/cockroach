        ...
        public constructor(nodeId?: string) {
          ...
          this._addChart(
            Metrics.NewAxis(
              ...
              Metrics.Select.AvgRate(_nodeMetric("exec.success-count"))
                .sources(this.sources)
                .title("Success Calls")
              ).format(d3.format("d"))._title("Successes vs Errors").range([0]));
          ...
          this._addChart(
            Metrics.NewAxis(
              ...
              ).format(Utils.Format.Bytes)._title("Capacity").range([0]));
          ...
          this._addChart(
            Metrics.NewAxis(
              ...
              Metrics.Select.Avg(_sysMetric("cpu.sys.percent"))
                .sources(this.sources) // TODO: store sources vs node sources
                .title("CPU Sys %")
              ).format(d3.format(".2%"))._title("CPU").range([0, 1]).stacked(true));
